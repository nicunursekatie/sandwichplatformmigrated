...
// Add a new component for the chat hub that handles group selection
interface GroupMessagingProps {
  currentUser?: User;
}

export function GroupMessaging({ currentUser }: GroupMessagingProps) {
  const [selectedGroup, setSelectedGroup] = useState<{ id: number; name: string; description?: string } | null>(null);
  const { user } = useAuth();
  
  // Fetch available groups for the user
  const { data: groups = [], isLoading: groupsLoading } = useQuery<Array<{
    id: number;
    name: string;
    description?: string;
  }>>({
  ...
  // If a group is selected, show the conversation
  if (selectedGroup) {
    return (
      <GroupConversation
      ...
      <div className="flex-1 p-4">
        {groupsLoading ? (
          ...
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-xs">Contact an administrator to join a group</p>
          </div>
        ) : (
          <div className="space-y-2">
            {groups.map((group) => (
