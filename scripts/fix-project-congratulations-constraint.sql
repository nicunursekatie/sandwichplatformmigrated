...
FROM pg_constraint 
WHERE conrelid = 'project_congratulations'::regclass 
AND confrelid = 'projects'::regclass
AND contype = 'f';

-- Drop the existing foreign key constraint (replace 'constraint_name' with the actual name from above query)
-- Example: ALTER TABLE project_congratulations DROP CONSTRAINT project_congratulations_project_id_fkey;
...
-- Now you should be able to delete projects!