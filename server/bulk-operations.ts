...
export class BulkOperationsManager {
  ...
  static async validateDataIntegrity(): Promise<{
    ...
    try {
      ...
      const orphanedCollectionsResult = await db.execute(sql`
      ...
      `);
      const orphanedCollections = orphanedCollectionsResult.rows[0];
      ...
      const duplicateHostsResult = await db.execute(sql`
        SELECT COUNT(*) - COUNT(DISTINCT LOWER(TRIM(name))) as count
        FROM hosts
      `);
      const duplicateHosts = duplicateHostsResult.rows[0];
      ...
      const invalidDatesResult = await db.execute(sql`
        SELECT COUNT(*) as count
        FROM sandwich_collections
        WHERE collection_date IS NULL OR collection_date = ''
      `);
      const invalidDates = invalidDatesResult.rows[0];
      ...
      const invalidCountsResult = await db.execute(sql`
        SELECT COUNT(*) as count
        FROM sandwich_collections
        WHERE sandwich_count IS NULL OR sandwich_count <= 0
      `);
      const invalidCounts = invalidCountsResult.rows[0];
